...
<div class="flex justify-center items-center h-screen" style="background-color: #9DBDFF;">
    <div class="p-6 max-w-sm w-full" style="background-color: #FFD7C4; box-shadow: 0px 4px 6px rgba(0, 0, 0, 0.1); border-radius: 8px;">
        <div class="flex justify-center items-center">
            <span style="color: #7695FF; font-weight: bold; font-size: 24px;">RESET PASSWORD</span>
        </div>
        @if (session('status'))
        ...
        <form class="mt-4" action="{{ route('password.email') }}" method="POST">
            ...
            <label class="block mt-3">
                ...
                <input type="email" name="email" value="{{ old('email') }}" class="form-input mt-1 block w-full rounded-md border-[#7695FF]" placeholder="Alamat Email">
                @error('email')
            ...
            <div class="mt-6">
                <button type="submit" class="py-2 px-4 text-center" style="background-color: #7695FF; color: white; border-radius: 8px; width: 100%;">SEND PASSWORD RESET LINK</button>
            </div>
        </form>
    </div>
</div>
