        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden">
            ...
            <div class="p-6 bg-gradient-to-r from-blue-600 to-blue-800 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    <h2 class="text-xl font-semibold text-white">Edit Galeri</h2>
            ...
            <!-- Form -->
            <form action="{{ route('admin.galery.update', $galery->id) }}"
                  method="POST"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <select name="kategori_id"
                            ...
                            @foreach($categories as $kategori)
                                <option value="{{ $kategori->id }}"
                                        {{ old('kategori_id', $galery->kategori_id) == $kategori->id ? 'selected' : '' }}>
                                    {{ $kategori->judul }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-span-2">
                        ...
                        <textarea name="deskripsi"
                                  rows="4"
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 transition-colors"
                                  required>{{ old('deskripsi', $galery->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
 ...
 <div class="mb-4">
                    ...
                    <div>
                        ...
                        <select name="status"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 transition-colors"
                                required>
                            <option value="active" {{ old('status', $galery->status) == 'active' ? 'selected' : '' }}>
                                Aktif
                            </option>
                            <option value="inactive" {{ old('status', $galery->status) == 'inactive' ? 'selected' : '' }}>
                                Tidak Aktif
                            </option>
                        </select>
                        @error('status')
                    ...
                    <div>
                        ...
                        <input type="date"
                               name="tanggal"
                               value="{{ old('tanggal', $galery->tanggal) }}"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 transition-colors"
                               required>
                        @error('tanggal')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
